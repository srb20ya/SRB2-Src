...
//-----------------------------------------------------------------------------
//
// $Id: i_sound_sdl.c,v 1.2 2000/04/20 12:12:11 emanne Exp $
//
...
//
// $Log: i_sound_sdl.c,v $
// Revision 1.2  2000/04/20 12:12:11  emanne
// The last bugs with music going away when pausing and with sfx volume are gone
//
//
// DESCRIPTION:
...
#define nomusicparm nomusic
/* Compatibility with nomusic from legacy */


static const char
rcsid[] = "$Id: i_sound_sdl.c,v 1.2 2000/04/20 12:12:11 emanne Exp $";
...
#define snd_SfxVolume cv_soundvolume.value
  ...
  int		priority )
{
  ...
  priority = 0;
    // Debug.
  /*  fprintf( stderr, "starting sound %d vol %d sep %d pitch %d priority %d",
...
{ 
  ...
  /* Comprends pas pourquoi le SDL_Init avait été commenté... */
  ...
  fprintf( stderr, "I_InitSound: (%d sfx)",NUMSFX);
  ...
  { 
      ...
      if (!S_sfx[i].link)
	{
	  ...
	  //	  printf("Loaded %s id %d longueur... ",S_sfx[i].name,i);
