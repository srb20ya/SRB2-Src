...
//-----------------------------------------------------------------------------
//
// $Id: 3dmath.c,v 1.3 2000/03/29 19:39:49 bpereira Exp $
//
    ...
    for(a=0; a<numpoints; a++)
    {
        ...
        if ( curin != nextin )
        {
            if (curdot >= nextdot)
            {
                ...
                poutvert->oow = pinvert->oow + scale * (inVerts[nextvert].oow - pinvert->oow );
            ...
            else
            {
                ...
                poutvert->oow = inVerts[nextvert].oow + scale * (pinvert->oow - inVerts[nextvert].oow );
...
int ClipToFrustum (FOutVector *inVerts, FOutVector *outVerts, int nrInVerts)
{
    ...
    for (i=0; i<nrInVerts; i++)
    {
        ...
        deltaz = nextvert->oow - pinvert->oow;
        if ( ClipT( -deltax - deltaz, pinvert->x + pinvert->oow, &tl, &tu ) ) {
            if ( ClipT( deltax - deltaz, pinvert->oow - pinvert->x, &tl, &tu ) ) {
                ...
                if ( tl > 0 ) {
                    ...
                    poutvert->oow = pinvert->oow + tl * deltaz;
                ...
                if ( tu < 1 ) {
                    ...
                    poutvert->oow = pinvert->oow + tu * deltaz;
    ...
    for (i=0; i<xClipVerts; i++)
    {
        ...
        deltaz = nextvert->oow - pinvert->oow;
        deltay = nextvert->y - pinvert->y;
        if ( ClipT( -deltay - deltaz, pinvert->y + pinvert->oow, &tl, &tu ) ) {
            if ( ClipT( deltay - deltaz, pinvert->oow - pinvert->y, &tl, &tu ) ) {
                deltax = nextvert->x - pinvert->x;
                // clipped start point
                if ( tl > 0 ) {
                    poutvert->x = pinvert->x + tl * deltax;
