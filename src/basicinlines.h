...
/// \file
/// \brief This file is based on pragmas.h from Ken Silverman's original Build
...
#define INT64  int
#else
"Warning, need 64 bit type"
#endif

static inline fixed_t Scale (fixed_t a, fixed_t b, fixed_t c)
{
	return (fixed_t)(((INT64) a * (INT64) b) / c);
}
...
#define MAKECONSTMulScale(s) \
	static inline SDWORD MulScale##s (SDWORD a, SDWORD b) \
	{ \
		return (fixed_t)(((INT64) a * (INT64) b) >> s); \
	}
MAKECONSTMulScale(1)
MAKECONSTMulScale(2)
MAKECONSTMulScale(3)
...
static inline SDWORD DMulScale (SDWORD a, SDWORD b, SDWORD c, SDWORD d, SDWORD s)
{
	return (fixed_t)((((INT64) a * (INT64) b) + ((INT64) c * (INT64) d)) >> s);
}
...
#define MAKECONSTTMulScale(s) \
	static inline SDWORD TMulScale##s (SDWORD a, SDWORD b, SDWORD c, SDWORD d, SDWORD e, SDWORD f) \
	{ \
...
static inline void qinterpolatedown16 (SDWORD *out, DWORD count, SDWORD val, SDWORD delta)
{
	SDWORD odd = count;
	if((count >>= 1) != 0)
	{
...
/*
static inline void qinterpolatedown16short (short *out, DWORD count, SDWORD val, SDWORD delta)
{
	if((size_t)out & 2)
...
static inline SDWORD DivMod (DWORD num, SDWORD den, SDWORD *dmval)
{
	*dmval = num%den;
	return num/den;
}
	...
	//returns num%den, dmval = num/den
static inline SDWORD ModDiv (DWORD num, SDWORD den, SDWORD *dmval)
{
	*dmval = num/den;
	return num%den;
}
