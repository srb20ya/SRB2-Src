			...
			foreach ($SRB2MSRSS10->items as $item) 
			{
				...
				else
				{
					...
					if ( $published >= $lastupdatetime ) 
					{
						$description = $item['dc']['description'];
						$title = $item['title'];
						$irc->message(SMARTIRC_TYPE_CHANNEL, '#srb2fun'    , "New Server, Name: $title $description");
	...
	function ListGamesChan(&$irc, &$data)
	{
		...
		if ($fd)
		{
			...
			if ( (mktime($today['hours'],$today['minutes']-5,0,$today['mon'], $today['mday'], $today['year'])) >= $lastlisttime )
			{
				...
				if (strcasecmp($content, "No SRB2 Games are running, so why don't you start one?") >= 0)
				{
				/*
				switch (rand(0, 5)) 
					{
					case 0:
						break;
					case 1:
						$content = "$data->nick, go away, there are no games up!";
						break;
					case 2:
						$content = "Please don't do !gamelist again, or I will kick you!";
						break;
					case 3:
						$content = "Go get your own game list, I don't have any here. >.<";
						break;
					case 4:
						$content = "$data->nick, go away, you smell like Eggman";
						break;
					case 5:
						$content = "go away, $data->nick, just go away";
						break;
					//case 6:
					//	$content = "GO THE FSCK AWAY";
					//	break;
					}
				*/
				}
			...
			{
			//$now = mktime($today['hours'],$today['minutes']-5,0,$today['mon'], $today['mday'], $today['year']);
			//$irc->message(SMARTIRC_TYPE_CHANNEL, $data->channel, $now);
			}
		}
		else
		{
...
//$esperirc->registerActionhandler(SMARTIRC_TYPE_CHANNEL, '^!RSSgamelist', $bot, 'RSSListGames');
...
// Connect to EsperNet
$esperirc->connect('legend.esper.net', 6667);
// Reconnect as needed
