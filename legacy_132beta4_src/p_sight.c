...
//-----------------------------------------------------------------------------
//
// $Id: p_sight.c,v 1.4 2001/06/10 21:16:01 bpereira Exp $
//
...
boolean P_CheckSight( mobj_t* t1, mobj_t* t2 )
{
	...
	// Prevent SOME cases of looking through 3dfloors Tails 03-18-2002
	//
...
#ifdef FIXROVERBUGS
	if(t1->subsector == t2->subsector) // Both are in the same subsector
	{
		if(t1->subsector->sector->ffloors)
		{
			ffloor_t* rover;

			for(rover = t1->subsector->sector->ffloors; rover; rover = rover->next)
			{
				// Allow sight through water, fog, etc. Maybe in the future this
				// could check fog density and set a sight limit?
