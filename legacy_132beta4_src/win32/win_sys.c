...
//-----------------------------------------------------------------------------
//
// $Id: win_sys.c,v 1.19 2001/08/07 00:54:40 hurdler Exp $
//
...
static void signal_handler(int num)
{
    ...
    MessageBox(hWndMain,va("signal_handler() : %s",sigmsg),"SRB2 error",MB_OK|MB_ICONERROR); // Tails
...
void I_Error (char *error, ...)
{
    ...
    if(shutdowning)
    {
        ...
        if(errorcount>20)
        {
            MessageBox(hWndMain,txt,"SRB2 Recursive Error",MB_OK|MB_ICONERROR); // Tails
    ...
    MessageBox (hWndMain, txt, "SRB2 Error", MB_OK|MB_ICONERROR); // Tails
